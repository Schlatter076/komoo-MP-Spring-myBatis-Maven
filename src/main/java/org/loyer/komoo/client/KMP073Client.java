...
public class KMP073Client extends LoyerFrame {
  ...
  /** 本类日志记录对象 */
  private static final Logger logger = LoggerFactory.getLogger(KMP073Client.class);
  ...
  public static void getDataView(ApplicationContext ac, String type, String base, String baseType) {
    EventQueue.invokeLater(new Runnable() {
      ...
      @Override
      public void run() {
        KMP073Client win = new KMP073Client(ac, type, base, baseType);
  ...
  public KMP073Client(ApplicationContext ac, String type, String base, String baseType) {
  ...
  class TestThreadListener implements Runnable {
    ...
    public void run() {
      while (true) {
        ...
        if (com2HasData) {
          if (step == 8 || step == 10 || step == 12 || step == 14 || step == 16) {
            if (rec_data < 40) {
