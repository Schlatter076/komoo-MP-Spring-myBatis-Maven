...
public class MyLineChart {
  ...
  public DefaultCategoryDataset getDataSet(List<RecordData> list) {
    ...
    for (RecordData data : list) {
      set.setValue(data.getNg(), "NG", data.getDate());
      set.setValue(data.getSum(), "SUM", data.getDate());
      set.setValue(data.getOk(), "OK", data.getDate());
    }
    return set;
  }
   ...
   * @param tableName
   */
  public static void saveAsJPEG(List<RecordData> list) {
    MyLineChart lineChart = new MyLineChart(list);
    try {
      String path = "excl/";
      ...
      // 创建可写入的Excel工作簿
      String fileName = list.get(0).getName() + ".png";
