...
public class KMP017Client extends LoyerFrame {
  ...
  /** 本类日志记录对象 */
  private static final Logger logger = LoggerFactory.getLogger(KMP017Client.class);
  ...
  public static void getDataView(ApplicationContext ac, String type, String base, String baseType) {
    EventQueue.invokeLater(new Runnable() {
      ...
      @Override
      public void run() {
        KMP017Client win = new KMP017Client(ac, type, base, baseType);
  ...
  public KMP017Client(ApplicationContext ac, String type, String base, String baseType) {
  ...
  class TestThreadListener implements Runnable {
    ...
    public void run() {
      while (true) {
        ...
        if(com2HasData) {
          if(step == 3 || step == 5 || step == 7 || step == 10) {
            if(rec_data < 40) {
