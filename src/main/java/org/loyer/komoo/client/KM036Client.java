...
public class KM036Client extends LoyerFrame {
  ...
  /** 串口1接收数据标志位 */
  private volatile boolean com1HasData = false;
  /** 串口1缓冲区 */
  private volatile byte[] com1Bytes = new byte[11];
  /** 串口2接收数据标志位 */
  private volatile boolean com2HasData = false;
  /** 串口2缓冲区 */
  ...
  /** 指令和步数对应关系集合 */
  private Map<Integer, byte[]> commamdsMap;
  ...
  public KM036Client(ApplicationContext ac, String type, String base) {
    ...
    commamdsMap = new HashMap<>();
    commamdsMap.put(1, KM036Commands.SET_BUZZER);
    commamdsMap.put(2, KM036Commands.CLR_ALL);
    commamdsMap.put(3, KM036Commands.SET_WA);
    commamdsMap.put(4, KM036Commands.SET_SEAT);
    commamdsMap.put(5, KM036Commands.SET_WSTEP_1);
    commamdsMap.put(6, KM036Commands.SET_WSTEP_2);
    commamdsMap.put(7, KM036Commands.SET_WSTEP_3);
    commamdsMap.put(8, KM036Commands.SET_WSTEP_4);
    commamdsMap.put(9, KM036Commands.SET_WNOZZLE_1);
    commamdsMap.put(10, KM036Commands.SET_WNOZZLE_2);
    commamdsMap.put(11, KM036Commands.SET_WNOZZLE_3);
    commamdsMap.put(12, KM036Commands.SET_WNOZZLE_4);
    commamdsMap.put(13, KM036Commands.SET_WPUMP);
    commamdsMap.put(14, KM036Commands.SET_DEMOT);
    commamdsMap.put(15, KM036Commands.SET_WVALUE);
          ...
          if (isEquals(com1Bytes[i + 9], "20")) { // 下位机开始
            ...
            if (isEquals(com1Bytes[i + 3], "77")) { // 下位机请求指令发送
              ...
              if (stepCounter != oldStep) { // 防止指令发送重复
                oldStep = stepCounter;
                if (stepCounter == 1) {
                  SerialPortTools.writeBytes(COM[1], commamdsMap.get(stepCounter));
                  checkBuzzer = true;
  ...
  public void initTable2() {
    for (int i = 0; i < table.getRowCount() - 1; i++) {
      table.setValueAt("fail", i, 5); // 清空测试值
      table.setValueAt("NG", i, 7); // 清空测试结果
    }
  }
      ...
      while(true) {
        ...
        if (checkBuzzer && !spotButt.isSelected()) {
          if (com2HasData) {
            ...
            SerialPortTools.writeBytes(COM[1], KM036Commands.SET_BUZZER);
            if (buzzCounter > 3) {
              com2HasData = false;
              checkBuzzer = false;
              buzzCounter = 0;
